...
def config_model(method='cores', core_count=1):
...
def cpu_generator_model(api_client, running = False, id = '', method = 'cores', core_count = 1):
    gen = client.models.CpuGenerator()
    gen.running = running
    gen.config = config_model(method, core_count)
    gen.id = id
    return gen
