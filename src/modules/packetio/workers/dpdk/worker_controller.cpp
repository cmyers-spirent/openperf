                                      ...
                                      packets::generic_sink sink)
{
    ...
    if (contains_match(m_fib->get_interface_sinks(*port_idx, ifp), sink)) {
        return (tl::make_unexpected(EALREADY));
    }
    ...
    auto to_delete =
        m_fib->insert_interface_sink(*port_idx, ifp, std::move(sink));
    m_recycler->writer_add_gc_callback([to_delete]() { delete to_delete; });
                                           ...
                                           packets::generic_sink sink)
{
    ...
    auto to_delete =
        m_fib->remove_interface_sink(*port_idx, ifp, std::move(sink));
    m_recycler->writer_add_gc_callback([to_delete]() { delete to_delete; });
