#ifndef _OP_PACKET_STACK_GENERIC_STACK_HPP_
#define _OP_PACKET_STACK_GENERIC_STACK_HPP_
...
public:
    ...
    void delete_interface(std::string_view id) { m_self->delete_interface(id); }
...
private:
    struct stack_concept
    {
        ...
        virtual std::optional<packetio::interface::generic_interface>
        interface(std::string_view id) const = 0;
        virtual tl::expected<std::string, std::string>
        create_interface(const packetio::interface::config_data& config) = 0;
        virtual void delete_interface(std::string_view id) = 0;
        virtual std::unordered_map<std::string, stats_data> stats() const = 0;
    };

    template <typename Stack> struct stack_model final : stack_concept
    {
        stack_model(Stack s)
            : m_stack(std::move(s))
        {}

        std::string id() const override { return m_stack.id(); }
        ...
        std::optional<packetio::interface::generic_interface>
        interface(std::string_view id) const override
        {
            return m_stack.interface(id);
        }

        tl::expected<std::string, std::string> create_interface(
            const packetio::interface::config_data& config) override
        {
...
std::unique_ptr<swagger::v1::model::Stack>
make_swagger_stack(const generic_stack& stack);

} // namespace openperf::packet::stack

#endif /* _OP_PATCKET_STACK_GENERIC_STACK_HPP_ */
