...
static int handle_tcpip_timeout(packetio::event_loop::generic_event_loop& loop
...
static int handle_tcpip_message(packetio::event_loop::generic_event_loop& loop
                                __attribute__((unused)),
                                std::any arg)
{
    ...
    , m_timerfd(timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK))
{
    ...
    auto msg_id = m_client.add_task(packetio::workers::context::STACK,
                                    "stack API messages",
    ...
    auto timeout_id = m_client.add_task(packetio::workers::context::STACK,
                                        "stack timers",
                                        m_timerfd,
...
lwip::~lwip()
{
    ...
    for (auto& id : m_tasks) { m_client.del_task(id); }

    tcpip_input::instance().fini();
...
lwip& lwip::operator=(lwip&& other) noexcept
{
    if (this != &other) {
        ...
        std::swap(m_timerfd, other.m_timerfd);
    }
...
lwip::interface(std::string_view id) const
{
    return (m_interfaces.find(id) != m_interfaces.end()
                ? std::make_optional(packetio::interface::generic_interface(
                    make_netif_wrapper(m_interfaces.at(std::string(id)))))
                : std::nullopt);
}

tl::expected<std::string, std::string>
lwip::create_interface(const packetio::interface::config_data& config)
{
    ...
    auto port_index = m_client.get_port_index(config.port_id);
    if (!port_index) {
        return (tl::make_unexpected("Could not find port index for port id "
                                    + config.id));
    }

    auto tx_func = m_client.get_transmit_function(config.port_id);
    if (!tx_func) {
        return (tl::make_unexpected(
            "Could not find transmit function for port id " + config.id));
    }

    /* XXX: Interface creation can throw exceptions for a variety of reasons */
    try {
        auto ifp = std::make_unique<net_interface>(
            config.id, *port_index, config, *tx_func);
        auto success = m_client.add_interface(
            ifp->port_id(),
            packetio::interface::generic_interface(make_netif_wrapper(ifp)));
        if (!success) {
            return (tl::make_unexpected(strerror(success.error())));
        }
...
static stack::memory_stats_data make_memory_stats_data(const memp_desc* mem)
{
    return {.name = mem->desc,
            .available = packet_stack_memp_pool_avail(mem),
            .used = packet_stack_memp_pool_used(mem),
            .max = packet_stack_memp_pool_max(mem),
